...
if __name__ == "__main__":
    ...
    for settings, options, env_vars, build_requires, reference in builder.items:
        ...
        if not settings['compiler'] == "apple-clang" \
            ...
            pybind_options.update({'xmsgeom:pybind': True})
    ...
    for settings, options, env_vars, build_requires, reference in builder.items:
        ...
        if settings['compiler'] == 'Visual Studio' \
            ...
            xms_options.update({'xmsgeom:xms': True})
    ...
    for settings, options, env_vars, build_requires, reference in builder.items:
        # testing option
        if not options.get('xmsgeom:xms', False) and not options.get('xmsgeom:pybind', False):
            testing_options = dict(options)
            testing_options.update({'xmsgeom:testing': True})
